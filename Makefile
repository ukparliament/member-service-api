...
build :
	docker build -t $(IMAGE):$(VERSION) -t $(IMAGE):latest \
		--build-arg UKPDS_DATA_URI_PREFIX=http://id.ukpds.org --build-arg UKPDS_DATA_ENDPOINT=http://51.140.32.176/repositories/Master \
		.
...
dev:
	docker run -p 80:3000 -v ${PWD}:/opt/membersprototype-api $(IMAGE)
